<template>
  <div class="wrap" v-if="estate.meta">
    ...
    <Tab :estate="estate">
      ...
      <p class="text-xl font-medium text-black">Setle renovation link</p>
      ...
      <div class="flex items-center my-5 space-x-5">
        ...
        <label for="enable_setle" class="text-gray-700">Enable</label>
      </div>

      <table v-if="form.enable_setle" class="form-table" role="presentation">
        <tbody>
          <tr>
            <th scope="row">
              <label for="setle_link">Link</label>
            </th>
            <td>
              <input
                type="text"
                id="setle_link"
                v-model="form.setle_link"
                class="max-w-3xl regular-text"
                required
              />
            </td>
          </tr>
  ...
  methods: {
    ...
    fillForm() {
      if (this.estate.meta.custom_fields) {
        ...
        // Setle
        this.form.enable_setle = this.estate.meta.custom_fields.enable_setle;
        this.form.setle_link = this.estate.meta.custom_fields.setle_link;
        // Additional price
